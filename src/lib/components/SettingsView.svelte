<script lang="ts">
	...
	const platformConfigs = {
		macOS: {
			name: 'macOS',
			path: '/Applications/Prompt Forge.app/Contents/MacOS/prompt-forge'
		},
		Windows: {
			name: 'Windows',
			path: 'C:\\\\Program Files\\\\Prompt Forge\\\\prompt-forge.exe'
		},
		Linux: {
			name: 'Linux',
			path: '/usr/bin/prompt-forge'
		}
	};

	function getMcpConfig(path: string): string {
	...
	<div class="settings-sections">
		...
		<section class="settings-section card">
			...
			<p class="section-description">
				Connect Prompt Forge to Claude Code via MCP (Model Context Protocol) to access your agents,
			...
			<div class="mcp-status">
				<div class="status-indicator" class:running={$mcpStatus.running}>
					...
					<span class="status-text">{$mcpStatus.running ? 'Running' : 'Stopped'}</span>
				</div>
				<button
					class="btn"
					class:btn-primary={!$mcpStatus.running}
					class:btn-secondary={$mcpStatus.running}
					onclick={toggleMcpServer}
					disabled={$loadingState.mcp}
				>
					{#if $loadingState.mcp}
						...
					{:else}
			...
			{#if $mcpStatus.running}
				<div class="mcp-info">
					<div class="info-row">
						...
						<span class="info-value">{$mcpStatus.port}</span>
					</div>
					<div class="info-row">
						<span class="info-label">Available Tools:</span>
						<span class="info-value">{$mcpStatus.available_tools.length}</span>
					</div>
				</div>
			...
			<div class="mcp-connection-info">
				<h4>Connect to AI Assistants</h4>
				<p>Copy the configuration for your platform, then paste it into the appropriate config file:</p>

				<div class="platform-tabs">
					{#each Object.entries(platformConfigs) as [key, config]}
						<div class="platform-config">
							<div class="platform-header">
								...
								<button
									class="copy-btn"
									onclick={() =>
										copyToClipboard(getMcpConfig(config.path), `${config.name} config copied!`)}
								>
									Copy
								</button>
							</div>
							<pre><code>{getMcpConfig(config.path)}</code></pre>
						</div>
					{/each}
				</div>

				<div class="config-note">
					<strong>For Claude Code (CLI):</strong> Add to <code>~/.claude.json</code><br /><br />
					<strong>For VS Code Copilot:</strong> Add to <code>~/Library/Application Support/Code/User/mcp.json</code> (macOS)
					or <code>%APPDATA%\Code\User\mcp.json</code> (Windows). Create the file if it doesn't exist.<br /><br />
					<strong>Important:</strong> After adding the config, restart VS Code or run "Developer: Reload Window" from the command palette.
				</div>
			</div>
			...
			<div class="form-group">
				...
				<input
					id="mcp-port"
					type="number"
					bind:value={$settings.mcp_server_port}
					min="1024"
					max="65535"
				/>
				<p class="form-hint">The port the MCP server listens on (default: 3333)</p>
			</div>
		</section>
